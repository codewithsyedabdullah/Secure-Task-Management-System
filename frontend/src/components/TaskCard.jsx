...
const STATUS_BADGE  = { todo:'bg-[#21262d] text-[#8b949e]', in_progress:'bg-blue-900/50 text-blue-300', done:'bg-green-900/50 text-green-300', need_help:'bg-red-900/50 text-red-300', need_more_time:'bg-orange-900/50 text-orange-300' };
const PRIORITY_BADGE = { low:'bg-[#21262d] text-[#8b949e]', medium:'bg-yellow-900/50 text-yellow-300', high:'bg-red-900/50 text-red-300' };
...
const STATUS_OPTS = [
  ...
  { value:'todo',           label:'↩ To Do',       cls:'bg-[#21262d] text-[#8b949e] hover:bg-[#30363d]' },
];
...
export default function TaskCard({ task, onEdit, onDelete, onStatusUpdate }) {
  ...
  return (
    <div className="bg-[#161b22] border border-[#30363d] rounded-xl p-4 hover:border-[#484f58] transition-all hover:shadow-lg hover:shadow-black/20">
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        {isTeamCreator && (
          <div className="flex gap-1 shrink-0">
            <button onClick={() => onEdit(task)} title="Edit" className="text-[#484f58] hover:text-blue-400 p-1 rounded transition-colors">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
            </button>
            <button onClick={() => onDelete(task.id)} title="Delete" className="text-[#484f58] hover:text-red-400 p-1 rounded transition-colors">
      ...
      {task.description && <p className="text-xs text-[#8b949e] mb-3 line-clamp-2 leading-relaxed">{task.description}</p>}

      <div className="flex flex-wrap gap-1.5 mb-3">
        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_BADGE[task.status]||STATUS_BADGE.todo}`}>{STATUS_LABELS[task.status]||task.status}</span>
        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${PRIORITY_BADGE[task.priority]||PRIORITY_BADGE.medium}`}>{task.priority?.charAt(0).toUpperCase()+task.priority?.slice(1)}</span>
        {task.team_name && <span className="bg-purple-900/50 text-purple-300 text-xs font-medium px-2 py-0.5 rounded-full">{task.team_name}</span>}
      </div>
            ...
            <>
              <div className="flex -space-x-1.5">
                {task.assignees.slice(0,3).map(a => (
                  <div key={a.id} title={a.username} className="w-5 h-5 rounded-full bg-blue-600 border border-[#161b22] flex items-center justify-center text-white font-semibold" style={{fontSize:'9px'}}>{a.username[0].toUpperCase()}</div>
                ))}
                {task.assignees.length > 3 && <div className="w-5 h-5 rounded-full bg-[#21262d] border border-[#161b22] flex items-center justify-center text-[#8b949e]" style={{fontSize:'9px'}}>+{task.assignees.length-3}</div>}
              </div>
      ...
      {/* Status actions — visible to assignees AND team creator */}
      {canUpdateStatus && (
        <div className="border-t border-[#21262d] pt-3">
          <p className="text-xs text-[#484f58] mb-2 font-medium">Update status:</p>
          <div className="flex flex-wrap gap-1.5">
            {STATUS_OPTS.map(opt => (
              <button key={opt.value} disabled={updatingStatus||task.status===opt.value} onClick={() => handleStatusChange(opt.value)}
