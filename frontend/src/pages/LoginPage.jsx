...
const FEATURES = [
  { icon: '⚙️', title: 'Adaptable performance', desc: 'Effortlessly adjusts to your team needs, boosting efficiency and simplifying your workflow.' },
  { icon: '🛠️', title: 'Built to last', desc: 'Unmatched reliability designed for long-term team collaboration.' },
  { icon: '👍', title: 'Great user experience', desc: 'Intuitive interface that gets out of your way so you can focus on what matters.' },
  { icon: '✨', title: 'Innovative functionality', desc: 'Features that address your evolving team management needs.' },
];
...
export default function LoginPage() {
  ...
  const handleSubmit = async ev => {
    ...
    catch (err) { setServerError(err.response?.data?.error || 'Sign in failed.'); }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d1117', fontFamily: 'DM Sans, system-ui, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 16px' }}>
      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: '80px', width: '100%', maxWidth: '900px', flexWrap: 'wrap' }}>

        {/* Left features panel */}
        <div style={{ flex: '1 1 280px', maxWidth: '380px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontWeight: 600, color: '#c9d1d9', fontSize: '16px' }}>TaskManager</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {FEATURES.map(f => (
              <div key={f.title} style={{ display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
                ...
                <div>
                  <p style={{ color: '#c9d1d9', fontWeight: 600, fontSize: '14px', margin: '0 0 4px' }}>{f.title}</p>
                  <p style={{ color: '#8b949e', fontSize: '13px', margin: 0, lineHeight: '1.5' }}>{f.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Sign in card */}
        <div style={{ flex: '0 0 auto', width: '100%', maxWidth: '380px', background: '#161b22', border: '1px solid #30363d', borderRadius: '16px', padding: '32px' }}>
          <h1 style={{ fontSize: '28px', fontWeight: 700, color: '#ffffff', margin: '0 0 24px', letterSpacing: '-0.5px' }}>Sign in</h1>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <p style={{ textAlign: 'center', fontSize: '13px', color: '#8b949e', margin: 0 }}>
              ...
              <Link to="/register" style={{ color: '#58a6ff', fontWeight: 600, textDecoration: 'none' }}>Sign up</Link>
            </p>
          </form>
        </div>
      </div>
