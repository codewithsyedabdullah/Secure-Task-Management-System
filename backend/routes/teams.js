...
// POST /teams/:id/invite — stub: invite a user by email (no SMTP, logs intent)
router.post(
  '/:id/invite',
  [
    param('id').isInt(),
  ...
  async (req, res) => {
    ...
    try {
      ...
      const existingUser = await pool.query('SELECT id FROM users WHERE email = $1', [req.body.email]);
      ...
      // Stub: log the invite (no real email sent)
      console.log(`[INVITE STUB] Invite to join "${teamResult.rows[0].name}" sent to: ${req.body.email} by user ${req.user.id}`);

      res.status(200).json({
        message: `Invite sent to ${req.body.email}. They'll receive an email to join "${teamResult.rows[0].name}".`,
        stubNote: 'Email delivery is stubbed — no SMTP configured.',
      });
    } catch (err) {
